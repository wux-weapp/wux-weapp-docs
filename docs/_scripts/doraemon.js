(function() {
	 ...
	 */
	function onResize() {
		let hide = navigator.userAgent.match(/(phone|pad|pod|iPhone|iPod|ios|iPad|Android|Mobile|BlackBerry|IEMobile|MQQBrowser|JUC|Fennec|wOSBrowser|BrowserNG|WebOS|Symbian|Windows Phone)/i)
		if (observer && div) {
			observer.disconnect()
	...
	function getCssString(cssText, selector, callback) {
		...
		let start = selector + '{'
		let start$ = selector.replace('.', '\\.') + '{'
		let end$ = '}'
		let reg = new RegExp(start$ + '.*?' + end$, 'g')
		let match = cssText.match(reg)
		if (match) {
			if (!match[0]) return ''
			let result = match.map(function(v) {
	...
	function deep(childList, cssText, callback) {
		childList.forEach(function(child) {
			...
			if (child.node === 'element') {
				...
				if (!className) return
				let style = getCssString(cssText, '.' + className, callback)
				let value = style && style.split(';')
